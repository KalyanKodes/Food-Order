...
export default function App() {
  ...
  const updateQuantity = (itemName, operation, singleItemPrice, path) => {
    console.log(itemName, operation, singleItemPrice);
    let updatedCart = [];
    let status = false;
  ...
  useEffect(() => {
    ...
    cartDetails.forEach((item) => {
      item.totalPrice = item.quantity * item.singleItemPrice;
  ...
  return (
    <>
      {/* Order Confirn */}
      {confirmOrder && (
        <div className="confirm-order-outer-wrapper" onClick={revertConfirm}>
          <div className="confirm-order-inner-wrapper">
            <button onClick={revertConfirm} className="revert-changes">
              X
            </button>
            <svg
              width="48"
            ...
            <button onClick={resetAll} id="start-new-order-button">
              Start new Order
            </button>
          </div>
        </div>
      ...
      >
        <h1 className="cart-heading">Your Cart ({cartDetails.length})</h1>
        ...
        {cartDetails.length !== 0 ? (
          <>
            {cartDetails.map((item, i) => (
              <CartItem
                ...
                totalPrice={item.quantity * item.singleItemPrice}
                quantity={item.quantity}
                key={i}
                stateChanger={removeFromCart}
              />
            ))}
            <div className="order-total">
              <p>Order Total</p>
            ...
            <button id="confirm-order" onClick={handleClick}>
              Confirm Order
            </button>
          </>
        ) : (
          <>
            ...
            <p>Your added items will display here.</p>
          </>
        )}
      </div>
    </>
